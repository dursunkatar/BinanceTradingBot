    ...
    public static partial class Extensions
    {
        public static Bband Bbands(this List<Candle> source, int period = 5, double devUp = 2, double devDown = 2, TicTacTec.TA.Library.Core.MAType type = TicTacTec.TA.Library.Core.MAType.Sma)
        {
            ...
            if (bbands == TicTacTec.TA.Library.Core.RetCode.Success)
            {
                return new Bband()
                {
                    UpperBand = IndicatorHelper.FixIndicatorOrdering(upperValues.ToList(), outBegIdx, outNbElement),
                    MiddleBand = IndicatorHelper.FixIndicatorOrdering(middleValues.ToList(), outBegIdx, outNbElement),
                    LowerBand = IndicatorHelper.FixIndicatorOrdering(lowerValues.ToList(), outBegIdx, outNbElement)
                };
            }
